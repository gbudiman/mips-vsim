function cache(e, b, a, wb, n) {
	...
	$('#general_' + this.name).text(generalInfo);
}
...
cache.prototype.visual = function() {
	...
	for (s = 0; s < this.associativity; s++) {
		if (this.writeback) {
			myString += 'Set ' + s.pad(5, ' ') + ': V L D    Tag   =>';
		}
		else {
		...
		for (i = 0; i < this.entrySize; i++) {
			...
			if (this.writeback) {
				myString += this.content[s][i]['dirty'] + ' ';
			}
...
cache.prototype.consumeSignal = function(bundle, address, data, memoryObject, target) {
	if (bundle['memRead'] == true) {
		return this.read(address, memoryObject, target);
	}
...
function cacheBlock(blockBits, setIndex, wb) {
	var myBlock = Array();
	var blockSize = Math.pow(2, blockBits);
	myBlock['valid'] = false;
	myBlock['LRU'] = setIndex;
	myBlock['tag'] = 0;
	if (wb) { myBlock['dirty'] = 0;}
...
function cacheEntry(entryBits, blockBits, setIndex, wb) {
	var mySet = new Array();
	var entrySize = Math.pow(2, entryBits);
	for (j = 0; j < entrySize; j++) {
		mySet[j] = new cacheBlock(blockBits, setIndex, wb);
	}
	return mySet;
}

function cacheFrame(e, b, a, wb) {
	var myCache = new Array();
	for (i = 0; i < a; i++) {
		myCache[i] = new cacheEntry(e, b, i, wb);
	}
	return myCache;
}
